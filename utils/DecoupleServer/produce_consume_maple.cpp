...
#include "math.h"
#include "../../libs/decoupling/include/dec_decoupling.h"
#include <stdio.h> 
...
#define MAX_THREADS 1200 // Because the numba pipeline has issues with dynamic allocation
...
char DECOUPLING_MODE = 'd';
int DEC_NUM_THREADS = atoi(getenv("DECADES_THREAD_NUM"));
int DEC_NUM_CONSUMERS = atoi(getenv("DECADES_NUM_CONSUMERS"));
...
uint64_t get_producer_qid(int data = NAN){
  ...
  // desc (1:1): 
  case 'd' :
    qid = (uint64_t) omp_get_thread_num();
  // graphattack (1:1):
  case 'g': 
    qid = (uint64_t) omp_get_thread_num();
  // default (1:1):
...
uint64_t get_consumer_qid(){
  ...
  // }
  uint32_t qid = (uint32_t) omp_get_thread_num() - DEC_NUM_PRODUCERS;
  return qid;
}
...
void _desc_compute_produce_i32(int val) {
  ...
  // ASK TYLER: x is int but dec_produce only takes uint:
  // Can I just feed it in and let it inpterpret as it needs and read back?
...
void _desc_compute_produce_ptr(void *x) {
  ...
  u.v = x;
  dec_open_producer(qid);
  // ASK TYLER: x is pointer but dec_produce only takes uint: 
...
void _desc_supply_produce_ptr(void *addr) {
  ...
  u64 u;
  // CHECK THIS WITH TYLER: *addr or addr?
  u.v = addr;
  dec_open_producer(qid);
  dec_produce64(qid, u.ui);
  dec_close_producer(qid);
...
void desc_supply_alu_rmw_fetchadd_float(float* addr, float to_add, int label) {
  ...
  // THIS NEEDS TO BE CHECKED with TYLER!!
  uint64_t qid = get_producer_qid();
  u32 u_to_add;
  u_to_add.f = to_add;
...
void desc_init(int num_pairs) {
  ...
  int threads = DEC_NUM_CONSUMERS + DEC_NUM_PRODUCERS;
