...
public:
  ...
  bool VisitStmt(Stmt *s) {
	...
	if (DECADES_decoupled) {
	  DECADES_thread_number *= 1;
	} else if (DECADES_biscuit) {
          DECADES_thread_number = 2;
          compute_kernel_name = "_kernel_";
          supply_kernel_name = "_kernel_biscuit";
        }
	...
	for (int i = 0; i < ce->getNumArgs() - 2; i++) {
	  ...
	  supply_args.append(",");
        ...
        if (DECADES_biscuit) {
          args.append("0,1");
          supply_args = "0";
        } else if (DECADES_decoupled){
	  args.append("DECADES_tid,");
	  supply_args.append("(DECADES_tid - (omp_get_num_threads()/2)),");
	  args.append("(omp_get_num_threads()/2)");
	  supply_args.append("(omp_get_num_threads()/2)");
	}else{
	  args.append("DECADES_tid,");
	  supply_args.append("DECADES_tid-DECADES_num_consumers,");
	  args.append("(omp_get_num_threads())");
	  supply_args.append("(omp_get_num_threads()-DECADES_num_consumers)");
	}

        std::string DECADES_COMPUTE_CALL = compute_kernel_name + "(" + args + ");";
	std::string DECADES_SUPPLY_CALL = supply_kernel_name + "(" + supply_args + ");";

	std::string DECADES_consumer_number_str = std::to_string(DECADES_num_consumers);
	
	if (DECADES_decoupled) {
	  SSBefore << "if (DECADES_tid < " << DECADES_consumer_number_str << ") {\n";
	  SSBefore << DECADES_COMPUTE_CALL + "\n}\n";
	  SSBefore << "else {\n";
	  SSBefore << DECADES_SUPPLY_CALL + "\n}\n";
	} else if (DECADES_biscuit) {
          SSBefore << "if (DECADES_tid == 0) {\n";
          SSBefore << DECADES_COMPUTE_CALL + "\n} else if (DECADES_tid == 1) {\n";
          SSBefore << DECADES_SUPPLY_CALL + "\n}\n";
	} else {
	  SSBefore << DECADES_COMPUTE_CALL << "\n";
	}
	...
	ST = e->getEndLoc().getLocWithOffset(2);
  ...
  bool VisitFunctionDecl(FunctionDecl *f) {
    ...
    if (f->hasBody()) {
      ...
      if (is_kernel_function(FuncName) && !(DECADES_biscuit)) {
	...
	// Add comment before
	std::stringstream SSBefore;
	SSBefore << "atomic_int fences[10000];" << std::endl;
...
int main(int argc, const char **argv) {
  ...
  auto env_dec_biscuit = getenv("DECADES_BISCUIT");
  auto env_dec_num_consumers = getenv("DECADES_NUM_CONSUMERS");
  
  if (env_dec_thread_num) {
    DECADES_thread_number = atoi(env_dec_thread_num);
  }

  if (env_dec_num_consumers) {
    DECADES_num_consumers = atoi(env_dec_num_consumers);
  }
  else {
    DECADES_num_consumers = atoi(env_dec_thread_num)/2;
  }
  ...
  if (env_dec_biscuit) {
    if (atoi(env_dec_biscuit) == 1) {
      DECADES_biscuit = true;
    }
  }
